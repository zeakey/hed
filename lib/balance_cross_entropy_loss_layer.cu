...
__global__ void BalanceCrossEntropyLossForwardGPU(const int nthreads,
...
template <typename Dtype>
__global__ void BalanceCrossEntropyLossIgnoreDiffGPU(const int count,
...
void BalanceCrossEntropyLossLayer<Dtype>::Forward_gpu(
  ...
  BalanceCrossEntropyLossForwardGPU<Dtype><<<CAFFE_GET_BLOCKS(count),
...
void BalanceCrossEntropyLossLayer<Dtype>::Backward_gpu(
  ...
  if (propagate_down[0]) {
    ...
    if (has_ignore_label_) {
      // NOLINT_NEXT_LINE(whitespace/operators)
      BalanceCrossEntropyLossIgnoreDiffGPU<Dtype><<<CAFFE_GET_BLOCKS(count),
